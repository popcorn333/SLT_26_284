...
@BackboneRegistry.register("ncsnpp_v2_wote")
class NCSNpp_v2_wote(nn.Module):
    """NCSN++ model without time/noise-level embedding.

    The forward signature remains ``forward(self, x, y, t)`` for API
    compatibility, but ``t`` is intentionally ignored.
    """

    @staticmethod
    def add_argparse_args(parser):
        parser.add_argument("--nf", type=int, default=80)
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        # Keep ``embedding_type`` in the __init__ signature so old config files
        # can still instantiate this class. It is deliberately unused here.
        ...
        out_channels = 1  # score.real, score.imag
        self.output_layer = nn.Conv2d(in_channels, out_channels, 1)

        modules = []
        # No timestep/noise-level embedding layers in this variant.
        # ``fourier_scale`` and ``embedding_type`` are accepted only for
        # backwards compatibility with configs written for NCSNpp_v2.
        ...
        if resblock_type == 'ddpm':
            ResnetBlock = functools.partial(ResnetBlockDDPM, act=act,
                ...
                skip_rescale=skip_rescale, temb_dim=None)
    ...
    def forward(self, x, y, t):
        ...
        # ``t`` is intentionally ignored in this no-time-embedding variant.
        # Keep the argument so existing training/inference code can call
        # model(x, y, t) without modification.
