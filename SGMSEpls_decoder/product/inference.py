#only inference the last 5 saved epochs
import json
import re
import torch
import params
...
def main(cfg: DictConfig):
    ...
    for i in range(ending_epoch - 1, ending_epoch - 31, -1): #only inference the last 30 saved epochs
        y_mel = []
        #get integer parts of ith checkpoint filename
        checkpoint_name = _get_basename(checkpoint_files[i])
        number_part = get_integer_part(checkpoint_name)
        index = int(number_part)

        #load the sorted ith checkpoint file
        ...
        with torch.no_grad():
            for i, item in enumerate(valid_dataset):
                ...
                print(f'audio is saved to {cwd}/{split}/{cvt_dir}/Epoch_{index}/{basenames[i]}.wav')
