...
For Linear issue `TAN-6`, exclude the first-depth `fm` subfolder and everything
under it from all inventory, submission, training, inference, repair, metric,
WER, UTMOSv2, training-loss, validation, audit, and reporting work. Do not
submit new jobs for `fm`, wait for `fm` jobs, or include `fm` results when
deciding whether `TAN-6` is complete. This issue-specific exclusion takes
precedence over generic instructions below that describe scanning every
first-depth `run.sh` folder.
...
- `/mnt/parscratch/users/acp23xt/private/DTDM_Unet`
...
The target repository is `/mnt/parscratch/users/acp23xt/private/DTDM_Unet`; always run repository commands there. The Symphony-created workspace is only runtime scratch space. Preserve all pre-existing dirty-tree changes and never reset, revert, delete, or overwrite unrelated user work.
...
After every submitted run.sh has finished its training phase, extract the training loss for every epoch from every run/sweep and save the combined, clearly labelled records to `/mnt/parscratch/users/acp23xt/private/DTDM_Unet/training_loss.log`. Preserve experiment, sweep/configuration, epoch, and loss fields, validate coverage against the completed training outputs, and perform any non-trivial extraction through a dependency-aware Slurm job.

Before considering the inventory final, rescan first-depth folders for newly added run.sh files. The folder `scorex0` was added after the initial ten-folder inventory and must be included in run submission, downstream dependency dispatch, metrics, WER, UTMOSv2, training-loss aggregation, coverage validation, and final reports.
