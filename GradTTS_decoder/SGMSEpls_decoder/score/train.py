...
def main(cfg: DictConfig):
    ...
    model = GradTTS(cfg)
    model.to(device)
    out_size = fix_len_compatibility(2*cfg.data.sample_rate//256, 5)
...
############################################################################################
    ...
    for epoch in range(value, cfg.training.n_epochs):
        #################################################################################################################
        #only part of code added for updating train_encoder flag
        if epoch >= 500:
            cfg.training.train_encoder = True
            model.train_encoder = True # This line is crucial to ensure that the encoder is set to training mode when train_encoder is True..
        ##################################################################################################################
