...
class Diffusion(BaseModule):
    ...
    def ratio_mask(self, mu, n, n_steps):
        r = (n_steps-n)/n_steps
        ...
        keep_len = r.reshape(B, 1)
        for b in range(B):
            ratio = keep_len[b, 0].item()
            num_keep = int(ratio * L)
            # Randomly choose dimens indices to keep
            keep_indices = torch.randperm(L, device=device)[:num_keep]

            # Set kept indices to 1 across all length
    ...
    def forward_diffusion(self, X0, mask, mu, n, n_steps):
        ...
        Mask_unstructured = self.ratio_mask(mu, n, n_steps)
        Xn = X0 * Mask_unstructured
        Xn = torch.where(Xn == 0, mu, Xn)
    ...
    def reverse_diffusion(self, z, mask, mu, n_timesteps, stoc=False, spk=None):
        ...
        for n in range(n_timesteps, 0, -1):  # n_timesteps, ..., 1
            ...
            x0_est = self.estimator(xt.unsqueeze(1), mu.unsqueeze(1), n/n_timesteps)  #input N, Unet actually predicts M(N-1)
    ...
    def loss_t(self, x0, mask, mu, n, spk=None):
        ...
        x0_est = self.estimator(xt.unsqueeze(1), mu.unsqueeze(1), n/self.n_timesteps)
